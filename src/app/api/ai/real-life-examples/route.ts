...
export async function POST(request: NextRequest) {
  try {
    ...
    const { topic, context_country = 'Ethiopia' } = body;
    ...
    // Create cache key
    const cacheKey = getCacheKey({ topic, context_country });

    // Check cache first
    const cached = cache.get(cacheKey);
    if (cached && Date.now() - cached.timestamp < CACHE_DURATION) {
      console.log(`✅ [Cache HIT] Returning cached real-life examples for: ${topic}`);
      return NextResponse.json({
        ...cached.data,
        cached: true,
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: 'Failed to generate real-life examples', details: errorText },
    ...
    console.log(`💾 [Cached] Real-life examples for: ${topic} (Total cache size: ${cache.size})`);
    ...
    console.error('❌ Real-life examples API error:', error);
    return NextResponse.json(
