...
/** Department Head ↔ School Head chat. Local-only (no backend) — instant, no polling. */
export function DeptHeadSchoolHeadMessagesTab() {
  const {
    deptHeadMessages,
    currentUser,
    sendDeptHeadMessage,
  ...
  const thread = useMemo(
    () =>
      scope?.departmentId
        ? deptHeadMessages.filter((m) => m.departmentId === scope.departmentId)
        : [],
    [deptHeadMessages, scope?.departmentId],
  );

  useEffect(() => {
    if (scope?.departmentId) {
      markDeptHeadMessagesRead(scope.departmentId, 'department-head');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [scope?.departmentId, thread.length]);
  ...
  const handleSend = () => {
    if (!draft.trim() || !scope?.departmentId) return;
    setSending(true);
    try {
      sendDeptHeadMessage({
    ...
    <div className={`${aisCard} flex h-[min(70vh,720px)] min-h-[420px] flex-col overflow-hidden`}>
      <header className="flex h-16 shrink-0 items-center gap-3 border-b border-ais-card-border bg-ais-surface-container-low/40 px-4 sm:px-5">
        ...
        <div className="min-w-0">
          <p className="truncate text-sm font-bold text-ais-on-surface">School Head</p>
          <p className={aisBodySm}>Direct message · {myName}</p>
        </div>
      </header>
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          ...
          {thread.length === 0 ? (
            <div className="mx-4 mt-4 rounded-2xl bg-gradient-to-br from-ais-primary/8 via-ais-primary/5 to-transparent px-6 py-10 text-center sm:mx-6">
              ...
              <p className={`${aisBodySm} mx-auto mt-1.5 max-w-sm`}>
                Say hello or share a department update with the school head.
              </p>
            </div>
          ) : (
            thread.map((msg) => {
                ...
                >
                  <div
                    className={`mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-[11px] font-bold text-white ${avatarColor(
                      msg.senderId || (mine ? 'department-head' : 'school-head'),
                    )}`}
                  >
        ...
        <div className="flex shrink-0 items-end gap-2 border-t border-ais-card-border bg-ais-surface-container-low/30 px-4 py-3 sm:px-5">
          <textarea
            ...
            placeholder="Message the School Head…"
            value={draft}
