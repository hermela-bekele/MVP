...
export function MessageBubble({
  ...
  const canDelete = message.authorId === currentUserId || canModerate;
  const threadId = message.threadIdForRoot;
  const replyCount = message.threadReplyCount ?? 0;

  return (
    <div
      className={`group relative border-b border-ais-row-border px-4 py-3 transition-colors last:border-b-0 hover:bg-ais-row-hover ${
    ...
    >
      <div className="flex gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-1 whitespace-pre-wrap break-words text-sm leading-relaxed text-ais-on-surface/90">
                ...
                <button
                  ...
                  className={`inline-flex items-center gap-1 rounded-lg border px-2 py-0.5 text-xs font-semibold transition-colors ${
                    r.me
                      ...
                      : 'border-ais-card-border bg-ais-surface-container-low text-ais-on-surface-variant hover:border-ais-primary/20'
                  }`}
                >
                  <span>{r.emoji}</span>
                  <span>{r.count}</span>
          ...
          {showThreadLink && threadId && replyCount > 0 && (
            <button
              ...
              className="mt-2 inline-flex items-center gap-1.5 rounded-lg px-2 py-1 text-xs font-bold text-ais-primary transition-colors hover:bg-ais-primary/10"
            >
      ...
      <div className="absolute right-3 top-2 hidden items-center gap-0.5 rounded-lg border border-ais-card-border bg-white p-0.5 shadow-[0_1px_3px_rgba(15,23,42,0.08)] group-hover:flex">
