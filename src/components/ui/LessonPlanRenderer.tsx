  ...
  return (
    <div className={`lesson-plan-content space-y-6 ${className}`}>
      ...
      <div className="border-b border-border dark:border-gray-700 pb-3">
        <h2 className="text-xl font-bold text-foreground dark:text-gray-100">{content.title}</h2>
      </div>
      ...
      <div className="rounded-xl bg-gradient-to-br from-primary/5 via-accent/5 to-transparent dark:from-primary/10 dark:to-accent/10 p-5 border border-primary/10 dark:border-primary/20">
        <h3 className="text-sm font-semibold text-primary dark:text-primary mb-4 flex items-center gap-2">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
          </svg>
          Learning Objectives
        </h3>
        <ul className="space-y-3 text-sm text-foreground dark:text-gray-300">
      ...
      {content.activities && content.activities.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-base font-semibold text-foreground dark:text-gray-100 flex items-center gap-2">
          ...
          {content.activities.map((activity, idx) => {
            ...
            if (isMarkdown) {
              return (
                <div key={idx} className="rounded-xl border border-border dark:border-gray-700 bg-white dark:bg-gray-800 p-5">
            ...
            return (
              <div key={idx} className="rounded-xl border border-border dark:border-gray-700 bg-white dark:bg-gray-800 p-5 hover:shadow-lg dark:hover:shadow-gray-900/50 hover:border-primary/30 dark:hover:border-primary/40 transition-all duration-200 group">
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-xs font-medium text-muted-foreground dark:text-gray-400 bg-gradient-to-r from-accent/10 to-transparent dark:from-accent/20 px-3 py-1.5 rounded-full border border-accent/20 dark:border-accent/30">
                    {activity.duration}
                  </span>
                </div>
      ...
      <div className="rounded-xl border-l-4 border-l-accent bg-gradient-to-r from-accent/5 to-transparent dark:from-accent/10 p-5">
        <h3 className="text-sm font-semibold text-foreground dark:text-gray-100 mb-4 flex items-center gap-2">
      ...
      {/* Homework */}
      {content.homework && (
        <div className="rounded-xl bg-white dark:bg-gray-800 border border-border dark:border-gray-700 p-5 hover:shadow-md dark:hover:shadow-gray-900/50 transition-shadow">
          <h3 className="text-sm font-semibold text-foreground dark:text-gray-100 mb-3 flex items-center gap-2">
            <svg className="w-5 h-5 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
