  ...
  return (
    <div className={`markdown-content prose prose-sm md:prose-base max-w-none ${className}`}>
      <ReactMarkdown
        ...
        components={{
          h1: ({ node: _node, ...props }) => (
            <h1 className="text-2xl font-bold text-foreground mb-4 mt-6 pb-2 border-b border-border" {...props} />
          ),
          h2: ({ node: _node, ...props }) => (
            <h2 className="text-xl font-semibold text-foreground mb-3 mt-6 first:mt-0 pb-1 border-b border-border/60" {...props} />
          ),
          h3: ({ node: _node, ...props }) => (
