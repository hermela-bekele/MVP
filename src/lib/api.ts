...
export const MOE_DOCUMENT_AUDIENCES = [
  "All",
  "Regional",
  "Woredas",
  "Schools",
...
export const LEADERSHIP_ACTION_SEVERITIES = [
  "Low",
  "Medium",
  "High",
  "Critical",
] as const;
export const LEADERSHIP_ACTION_STATUSES = [
  "open",
  "in_progress",
  "resolved",
] as const;
...
export const MOE_THREAD_STATUSES = [
  "open",
  "awaiting_moe",
  "awaiting_school",
  "resolved",
...
export type TeacherReplacementReason =
  | "resignation"
  | "transfer"
  | "retirement"
  | "other";
export type TeacherReplacementStatus =
  | "pending"
  | "under_review"
  | "assigned"
  | "rejected"
...
function authHeaders(): Record<string, string> {
  ...
  try {
    const raw =
      localStorage.getItem("pts-session") ??
      sessionStorage.getItem("pts-session");
    if (!raw) return {};
...
export const api = {
  ...
  createRegion: (name: string) =>
    request<{ id: string; name: string }>("/regions", {
      method: "POST",
      body: JSON.stringify({ name }),
    }),
  updateRegion: (id: string, name: string) =>
    request<{ id: string; name: string }>(`/regions/${id}`, {
      method: "PATCH",
      body: JSON.stringify({ name }),
    }),
  ...
  updateSchoolIntegrationStatus: (id: string, status: "Active" | "Suspended") =>
    request(`/schools/${id}/integration-status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }),
  listMoeDocuments: (filters?: {
    category?: string;
    audience?: string;
    search?: string;
  }) => {
  ...
  uploadMoeDocument: (body: {
    ...
    request<MoeDocument>("/moe-documents", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  deleteMoeDocument: (id: string) =>
    request(`/moe-documents/${id}`, { method: "DELETE" }),
  listLeadershipActions: (filters?: {
    schoolId?: string;
    category?: "exception" | "improvement_initiative";
  }) => {
    const params = new URLSearchParams();
    ...
    if (filters?.category) params.set("category", filters.category);
    const qs = params.toString();
    return request<LeadershipAction[]>(
      `/leadership-actions${qs ? `?${qs}` : ""}`,
    );
  },
  createLeadershipAction: (
    body: Partial<LeadershipAction> & { issue: string },
  ) =>
    request<LeadershipAction>("/leadership-actions", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  updateLeadershipAction: (id: string, body: Partial<LeadershipAction>) =>
    request<LeadershipAction>(`/leadership-actions/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  createSchoolResource: (body: {
    ...
    request<SchoolResource>("/school-resources", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  deleteSchoolResource: (id: string) =>
    request(`/school-resources/${id}`, { method: "DELETE" }),
  ...
  deleteComplianceRequirement: (id: string) =>
    request(`/compliance-requirements/${id}`, { method: "DELETE" }),
  ...
  updateComplianceStatus: (body: {
    ...
    request<SchoolComplianceStatus>("/compliance-status", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  verifyComplianceStatus: (
    id: string,
    body: { status: "Verified" | "Rejected"; verificationNote?: string },
  ) =>
    request<SchoolComplianceStatus>(`/compliance-status/${id}/verify`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  createMoeMessageThread: (body: {
    subject: string;
    body: string;
    schoolId?: string;
  }) =>
    request<MoeMessageThread>("/moe-message-threads", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  listMoeThreadMessages: (threadId: string) =>
    request<MoeThreadMessage[]>(`/moe-message-threads/${threadId}/messages`),
  sendMoeThreadMessage: (threadId: string, body: string) =>
    request<MoeThreadMessage>(`/moe-message-threads/${threadId}/messages`, {
      method: "POST",
      body: JSON.stringify({ body }),
    }),
  updateMoeMessageThread: (
    threadId: string,
    body: { status?: string; markRead?: boolean },
  ) =>
    request<MoeMessageThread>(`/moe-message-threads/${threadId}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  getSchoolIntegrations: (schoolId: string) =>
    request(`/schools/${schoolId}/integrations`),
  updateSchoolIntegration: (
    schoolId: string,
    type: string,
    body: Record<string, unknown>,
  ) =>
    request(`/schools/${schoolId}/integrations/${type}`, {
      method: "PUT",
      body: JSON.stringify(body),
    }),
  ...
  listTeacherReplacementRequests: (query: { status?: string } = {}) => {
    const qs = query.status
      ...
      : "";
    return request<TeacherReplacementRequest[]>(
      `/teacher-replacement-requests${qs}`,
    );
  },
  ...
  assignTeacherReplacement: (
    id: string,
    body: { assignedTeacherId: string; moeNotes?: string },
  ) =>
    request<TeacherReplacementRequest>(
  ...
  processPayroll: (employeeId: string, month: string) =>
    request("/hr/payroll/process", {
      method: "POST",
      body: JSON.stringify({ employeeId, month }),
    }),
  updatePayrollStatus: (id: string, status: string) =>
    request(`/hr/payroll/${id}/status`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }),
  ...
  createTeacherLessonAdjustment: (body: Record<string, unknown>) =>
    request("/teacher-lesson-adjustments", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  listMyTeacherLessonAdjustments: () =>
    request("/teacher-lesson-adjustments/mine"),
  ...
  updateDeptAnnualLessonPlan: (id: string, body: Record<string, unknown>) =>
    request(`/lesson-plans/${id}/annual`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  listAssessmentReviewers: (departmentId?: string) =>
    request(
      `/assessment-reviewers${departmentId ? `?departmentId=${departmentId}` : ""}`,
    ),
  ...
  assignTrainingPlan: (
    ...
    request(`/training-plans/${planId}/assignments`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  updateTrainingPlanAssignment: (
    id: string,
    body: { attended?: boolean; impactRating?: number; impactNotes?: string },
  ) =>
    request(`/training-plan-assignments/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  submitSelfAssessment: (body: Record<string, unknown>) =>
    request("/teacher-self-assessments", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  assignTrainingModule: (body: Record<string, unknown>) =>
    request("/teacher-training-assignments", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  updateTrainingAssignmentStatus: (id: string, status: string) =>
    request(`/teacher-training-assignments/${id}`, {
      method: "PATCH",
      body: JSON.stringify({ status }),
    }),
  updateTrainingAssignmentProgress: (
    ...
    request(`/teacher-training-assignments/${id}/progress`, {
      method: "PATCH",
      body: JSON.stringify(progress),
    }),
  listMyTrainingAssignments: () =>
    request("/teacher-training-assignments/mine"),
  ...
  recalculateGpa: (studentId: string) =>
    request<{ gpa: number }>(`/students/${studentId}/recalculate-gpa`, {
      method: "POST",
    }),
  ...
  createCommunityReply: (postId: string, body: Record<string, unknown>) =>
    request<import("@/lib/mockData").CommunityReply>(
  ...
  markCommunityNotificationRead: (id: string) =>
    request<{ ok: boolean }>(`/community/notifications/${id}/read`, {
      method: "POST",
    }),
  markAllCommunityNotificationsRead: () =>
    request<{ ok: boolean }>("/community/notifications/read-all", {
      method: "POST",
    }),
  ...
  listApplications: (schoolId?: string, status?: string) => {
    ...
    return request<AdmissionApplication[]>(
      `/admissions/applications${s ? `?${s}` : ""}`,
    );
  },
  myApplications: () =>
    request<AdmissionApplication[]>("/admissions/applications/mine"),
  getApplication: (id: string) =>
    request<AdmissionApplication>(`/admissions/applications/${id}`),
  updateApplication: (id: string, body: Record<string, unknown>) =>
    request(`/admissions/applications/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  forcePromoteWaitlist: (id: string) =>
    request(`/admissions/waitlist/${id}/force-promote`, {
      method: "POST",
      body: "{}",
    }),
  ...
  listCapacity: (schoolId?: string) =>
    request<GradeSectionCapacity[]>(
      `/admissions/capacity${schoolId ? `?schoolId=${schoolId}` : ""}`,
    ),
  updateCapacity: (id: string, body: { capacity: number }) =>
    request(`/admissions/capacity/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  getSchoolSettings: (schoolId: string) =>
    request(`/admissions/settings/${schoolId}`),
  ...
  listRegistrationForms: (schoolId: string) =>
    request<RegistrationFormTemplate[]>(
      `/admissions/registration-forms?schoolId=${schoolId}`,
    ),
  ...
  getFinanceDashboardSummary: (schoolId?: string) =>
    request<FinanceDashboardSummary>(
      `/finance/dashboard-summary${schoolId ? `?schoolId=${schoolId}` : ""}`,
    ),
  listFinancialYears: (schoolId?: string) =>
    request<FinancialYear[]>(
      `/finance/financial-years${schoolId ? `?schoolId=${schoolId}` : ""}`,
    ),
  createFinancialYear: (body: {
    ...
    request<FinancialYear>("/finance/financial-years", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  activateFinancialYear: (id: string) =>
    request<FinancialYear>(`/finance/financial-years/${id}/activate`, {
      method: "POST",
    }),
  closeFinancialYear: (id: string) =>
    request<FinancialYear>(`/finance/financial-years/${id}/close`, {
      method: "POST",
    }),
  listFinancialPeriods: (params?: {
    schoolId?: string;
    ...
    if (params?.financialYearId)
      q.set("financialYearId", params.financialYearId);
    const s = q.toString();
    return request<FinancialPeriod[]>(
      `/finance/financial-periods${s ? `?${s}` : ""}`,
    );
  },
  closeFinancialPeriod: (id: string) =>
    request<FinancialPeriod>(`/finance/financial-periods/${id}/close`, {
      method: "POST",
    }),
  reopenFinancialPeriod: (id: string) =>
    request<FinancialPeriod>(`/finance/financial-periods/${id}/reopen`, {
      method: "POST",
    }),
  listAccounts: (schoolId?: string) =>
    request<FinanceAccount[]>(
      `/finance/accounts${schoolId ? `?schoolId=${schoolId}` : ""}`,
    ),
  createAccount: (body: {
    schoolId?: string;
    code: string;
    ...
    request<FinanceAccount>("/finance/accounts", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  updateAccount: (
    ...
    request<FinanceAccount>(`/finance/accounts/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  getBudgetUtilization: (id: string) =>
    request<BudgetUtilization>(`/finance/budgets/${id}/utilization`),
  createBudget: (body: {
    schoolId?: string;
    financialYearId: string;
    name: string;
  }) =>
    request<BudgetWithLines>("/finance/budgets", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  addBudgetLine: (
    ...
    request<BudgetWithLines>(`/finance/budgets/${budgetId}/lines`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  updateBudgetLine: (
    lineId: string,
    body: { allocatedAmount?: number; notes?: string },
  ) =>
    request<BudgetWithLines>(`/finance/budget-lines/${lineId}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  submitBudget: (id: string) =>
    request<Budget>(`/finance/budgets/${id}/submit`, { method: "POST" }),
  approveBudget: (id: string) =>
    request<Budget>(`/finance/budgets/${id}/approve`, { method: "POST" }),
  rejectBudget: (id: string, reason: string) =>
    request<Budget>(`/finance/budgets/${id}/reject`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    }),
  returnBudget: (id: string, reason: string) =>
    request<Budget>(`/finance/budgets/${id}/return`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    }),
  reviseBudget: (id: string) =>
    request<BudgetWithLines>(`/finance/budgets/${id}/revise`, {
      method: "POST",
    }),
  createBudgetTransfer: (body: {
    ...
    request<BudgetTransfer>("/finance/budget-transfers", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  approveBudgetTransfer: (id: string) =>
    request<BudgetTransfer>(`/finance/budget-transfers/${id}/approve`, {
      method: "POST",
    }),
  ...
  submitExpense: (id: string) =>
    request<Expense>(`/finance/expenses/${id}/submit`, { method: "POST" }),
  approveExpense: (id: string) =>
    request<Expense>(`/finance/expenses/${id}/approve`, { method: "POST" }),
  rejectExpense: (id: string, reason: string) =>
    request<Expense>(`/finance/expenses/${id}/reject`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    }),
  returnExpense: (id: string, reason: string) =>
    request<Expense>(`/finance/expenses/${id}/return`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    }),
  cancelExpense: (id: string, reason: string) =>
    request<Expense>(`/finance/expenses/${id}/cancel`, {
      method: "POST",
      body: JSON.stringify({ reason }),
    }),
  payExpense: (
    id: string,
    body: { paymentMethod: PaymentMethod; paymentReference?: string },
  ) =>
    request<Expense>(`/finance/expenses/${id}/pay`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  listSuppliers: (schoolId?: string) =>
    request<Supplier[]>(
      `/finance/suppliers${schoolId ? `?schoolId=${schoolId}` : ""}`,
    ),
  createSupplier: (body: {
    schoolId?: string;
    name: string;
    ...
    request<Supplier>("/finance/suppliers", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  updateSupplier: (
    id: string,
    body: Partial<Omit<Supplier, "id" | "schoolId" | "createdAt">>,
  ) =>
    request<Supplier>(`/finance/suppliers/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  listSupplierInvoices: (params?: {
    ...
    return request<SupplierInvoice[]>(
      `/finance/supplier-invoices${s ? `?${s}` : ""}`,
    );
  },
  getSupplierInvoice: (id: string) =>
    request<SupplierInvoice>(`/finance/supplier-invoices/${id}`),
  createSupplierInvoice: (body: {
    ...
    request<SupplierInvoice>("/finance/supplier-invoices", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  submitSupplierInvoice: (id: string) =>
    request<SupplierInvoice>(`/finance/supplier-invoices/${id}/submit`, {
      method: "POST",
    }),
  approveSupplierInvoice: (id: string) =>
    request<SupplierInvoice>(`/finance/supplier-invoices/${id}/approve`, {
      method: "POST",
    }),
  ...
  recordSupplierPayment: (
    ...
    request<SupplierInvoice>(`/finance/supplier-invoices/${id}/payments`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  createAnnouncement: (body: Record<string, unknown>) =>
    request("/portal/announcements", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  updateAnnouncement: (id: string, body: Record<string, unknown>) =>
    request(`/portal/announcements/${id}`, {
      method: "PATCH",
      body: JSON.stringify(body),
    }),
  ...
  myTimetable: () =>
    request<Record<string, unknown>[]>("/portal/timetable/mine"),
  staffWorkload: (schoolId?: string) => {
    const qs = schoolId ? `?schoolId=${encodeURIComponent(schoolId)}` : "";
    return request<
      { teacherId: string; teacherName: string; periodsPerWeek: number }[]
    >(`/portal/timetable/workload${qs}`);
  },
  getMoeCalendar: () =>
    request<import("@/lib/mockData").MoeCalendarDraft | null>("/moe-calendar"),
  saveMoeCalendar: (body: {
    id?: string;
    academicYear: string;
    title: string;
    events: unknown[];
  }) =>
    request<import("@/lib/mockData").MoeCalendarDraft>("/moe-calendar", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  publishMoeCalendar: (id: string) =>
    request<import("@/lib/mockData").MoeCalendarDraft>(
      `/moe-calendar/${id}/publish`,
      { method: "PATCH" },
    ),
  portalDocuments: (studentId: string) =>
    request<Record<string, unknown>[]>(
      `/portal/documents?studentId=${studentId}`,
    ),
  ...
  portalAttendance: (studentId: string) =>
    request<Record<string, unknown>[]>(
      `/portal/attendance?studentId=${studentId}`,
    ),
  ...
  createPracticeSet: (body: Record<string, unknown>) =>
    request<{ id: string }>("/portal/practice-sets", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  createQuestion: (body: Record<string, unknown>) =>
    request<{ id: string }>("/portal/question-bank", {
      method: "POST",
      body: JSON.stringify(body),
    }),

  listMessageThreads: () =>
    request<MessageThread[]>("/portal/messages/threads"),
  createMessageThread: (body: Record<string, unknown>) =>
    request<{ id: string }>("/portal/messages/threads", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  sendPortalFeedback: (body: Record<string, unknown>) =>
    request<{ id: string }>("/portal/feedback", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  createCalendarEvent: (body: Record<string, unknown>) =>
    request<{ id: string }>("/portal/calendar", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  ...
  permissionCatalog: () =>
    request<{ code: string; label: string; module: string }[]>(
      "/permissions/catalog",
    ),
  getUserPermissions: (userId: string, schoolId?: string) =>
    request(
      `/permissions/users/${userId}${schoolId ? `?schoolId=${schoolId}` : ""}`,
    ),
  setUserPermission: (
    userId: string,
    body: {
    ...
    request(`/permissions/users/${userId}`, {
      method: "POST",
      body: JSON.stringify(body),
    }),
  getRolePermissions: (role: string, schoolId?: string) =>
    request(
      `/permissions/roles/${role}${schoolId ? `?schoolId=${schoolId}` : ""}`,
    ),
  setRolePermissions: (
    role: string,
    permissions: string[],
